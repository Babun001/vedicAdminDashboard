...
import { revenueChartData, planDistributionData, mockTransactions, mockReports } from "@/lib/mock-data";
  ...
  return (
    <div className="space-y-6 animate-[fadeIn_0.4s_ease]">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        ...
        <div className="xl:col-span-2 bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-gray-500">Monthly revenue — 2024</p>
            </div>
        ...
        <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
          ...
          <p className="text-xs text-gray-500 mb-4">
            Active subscriptions · {stats.activeSubscriptions} paid
          </p>
          ...
          <div className="space-y-2 mt-2">
            {planDistributionData.map((d) => (
              <div key={d.name} className="flex items-center justify-between text-xs">
                ...
                <span className="text-gray-900 font-medium">{d.value} users</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        ...
        {/* Recent Transactions — from live SSE customers */}
        <div className="bg-white border border-gray-200 rounded-2xl p-5 shadow-sm">
          ...
          <div className="space-y-3">
            {customers.slice(0, 5).length > 0
              ...
              : recentTransactions.map((tx) => (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <p className="text-sm text-gray-900 truncate">{tx.userName}</p>
            ...
            {recentReports.map((r) => (
              ...
              >
                ...
                <div className="flex items-center gap-2 shrink-0 ml-3">
                  <Badge className={getPlanColor(r.template)}>{r.template}</Badge>
                  <Badge className={getReportStatusColor(r.status)}>{r.status}</Badge>
                </div>
              </div>
            ))}
          </div>
